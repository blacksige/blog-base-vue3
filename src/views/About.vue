<template>
  ...
  <button ref="btn" @click="start">
      <span class="done">End!</span>
      <span class="send">Start</span>
      <svg
        id="Capa_1"
...
export default defineComponent({
  ...
  mounted () {
    ...
    const t1 = window.gsap.timeline({ paused: true, reversed: true })
    let watch = false
  ...
  methods: {
    start () {
      ...
      const timer = setInterval(() => {
        second--
        console.log(second)
        const ceil = Math.floor(Math.random() * 9)
        document.getElementsByClassName('box')[cache].style.backgroundColor = ''
        cache = ceil
        document.getElementsByClassName('box')[ceil].style.backgroundColor = '#ED662C'
        if (second === 0) {
          clearInterval(timer)
          setTimeout(() => {
            this.$notify({
              title: '提示',
              message: '今天不如试试  ' + this.textList[cache],
              duration: 2000
            })
          }, 300)
        }
